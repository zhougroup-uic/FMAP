...
function fmapsq(x, gr, sigma, qstart, qstep, nq, mol) {
    let qs = [];
    for (let i = 0; i < nq; i++) {
...
function usage(prog) {
    console.log(`Usage: nodejs ${prog} Bltz.txt diam qstrt qstep Nq Cmol`);
    console.log(`   Or  nodejs ${prog} Bltz.txt diam qstrt qstep Nq Cmas MW`);
}
...
if (require.main === module) {
    ...
    let mol;
    if (args.length == 7) {
        mol = parseFloat(args[6]) / 1000.0;
    } else {
        const mas = parseFloat(args[6]);
        const mw = parseFloat(args[7]) * 1000.0;
        mol = mas / mw;
    }
    const numd = constantsN_A * mol * 1e-27;
    const [qs, qm, PY] = fmapsq(x, gr, sigma, qstart, qstep, nq, mol);
    for (let i = 0; i < qs.length; i++) {
        console.log(qs[i].toFixed(3), (1.0 / (1.0 / PY[i] - qm[i] * numd)).toExponential(4), (qm[i] * numd).toExponential(4), PY[i].toExponential(4));
    }
}
